...
export const Eventos = () => {
  ...
  const handleSave = async (e: FormEvent) => {
    ...
    try {
      ...
      addToast(
        currentEvent.id ? 'Evento atualizado com sucesso!' : 'Evento cadastrado com sucesso!',
        'success'
      );
  ...
  const onDateChange = (e: ChangeEvent<HTMLInputElement>) =>
    setCurrentEvent({ ...currentEvent, date: `${e.target.value} ${timePart || '00:00'}`.trim() });
  const onTimeChange = (e: ChangeEvent<HTMLInputElement>) =>
    setCurrentEvent({ ...currentEvent, date: `${datePart || ''} ${e.target.value}`.trim() });
                  ...
                  filteredEvents.map((evt) => (
                    <tr key={evt.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-center">
                        <span
                          className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold border ${evt.status === 'Ativo' ? 'bg-green-100 text-green-800 border-green-200' : 'bg-gray-100 text-gray-800 border-gray-200'}`}
                        >
                          {evt.status}
                        </span>
                      </td>
